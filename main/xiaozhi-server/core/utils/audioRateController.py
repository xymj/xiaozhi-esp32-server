...
class AudioRateController:
    ...
    def reset(self):
        ...
        self.logger.bind(tag=TAG).debug("AudioRateController 已重置")
    ...
    def add_message(self, message_type, message_data=None):
        """添加控制消息到队列"""
        self.queue.append(("message", message_type, message_data))
    ...
    def get_queue_info(self):
        ...
        return {
            ...
            "elapsed_ms": self._get_elapsed_ms() if self.start_timestamp else 0
        }
    ...
    async def check_queue(self, send_audio_callback, send_message_callback=None):
        """
        检查队列并按时发送音频/消息

        Args:
            send_audio_callback: 发送音频的回调函数 async def(opus_packet)
            send_message_callback: 发送消息的回调函数 async def(message_type, message_data)
        """
        if self.start_timestamp is None:
            self.reset()
            ...
            if item_type == "audio":
                ...
                if elapsed_ms < output_ms:
                    ...
                    self.logger.bind(tag=TAG).debug(
                        f"音频速率控制: 等待 {wait_ms:.1f}ms "
                        f"(elapsed={elapsed_ms:.1f}ms, output={output_ms}ms)"
                    )
                ...
                self.queue.pop(0)
                message_type = item[1]
                message_data = item[2] if len(item) > 2 else None

                if send_message_callback:
                    try:
                        await send_message_callback(message_type, message_data)
                    except Exception as e:
                ...
                # 未知类型，移除
                self.queue.pop(0)
                self.logger.bind(tag=TAG).warning(f"移除未知类型的队列项: {item_type}")
